...
impl IntervalStore {
    ...
    pub fn full() -> Self {
        Self(std::vec![Interval::new_unchecked(0, u16::MAX)])
    }
...
pub(crate) type RunIterOwned = RunIter<std::vec::IntoIter<Interval>>;
pub(crate) type RunIterBorrowed<'a> = RunIter<core::slice::Iter<'a, Interval>>;

impl IntoIterator for IntervalStore {
    type Item = u16;
    type IntoIter = RunIter<std::vec::IntoIter<Interval>>;
...
impl<I> SliceIterator<I> for std::vec::IntoIter<I> {
    fn as_slice(&self) -> &[I] {
        std::vec::IntoIter::as_slice(self)
    }
}
...
#[cfg(never_test_roaring)]
mod tests {
    use super::*;

    #[test]
    fn insert_empty() {
        let mut interval_store = IntervalStore(std::vec![]);
        assert!(interval_store.insert(1));
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 1, end: 1 }]))
    }

    #[test]
    fn insert_consecutive_begin() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 0, end: 0 },]);
        assert!(interval_store.insert(1));
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 0, end: 1 }]))
    }

    #[test]
    fn insert_consecutive_end() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 1, end: 1 },]);
    ...
    fn insert_consecutive_begin_end() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 0, end: 2 }]))
    }
    ...
    #[test]
    fn insert_u16_max() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 0, end: 3 },]);
    ...
    fn insert_u16_max_consecutive() {
        let mut interval_store =
            IntervalStore(std::vec![Interval { start: 0, end: u16::MAX - 1 },]);
        interval_store.insert(u16::MAX);
        assert_eq!(
            interval_store,
            IntervalStore(std::vec![Interval { start: 0, end: u16::MAX },])
        )
    }
    ...
    fn insert_range_empty() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 1, end: 2 },]));
    }

    #[test]
    fn insert_range_overlap_begin() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 1, end: 20 }]);
        assert_eq!(interval_store.insert_range(5..=50), Interval::new_unchecked(21, 50).run_len());
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 1, end: 50 },]));
    }

    #[test]
    fn insert_range_overlap_end() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 10, end: 20 }]);
        assert_eq!(interval_store.insert_range(5..=15), Interval::new_unchecked(5, 9).run_len());
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 5, end: 20 },]));
    }
    ...
    fn insert_range_overlap_begin_end() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 10, end: 60 },]));
    }
    ...
    fn insert_range_concescutive_begin() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 10, end: 50 },]));
    }
    ...
    fn insert_range_concescutive_end() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 50, end: 70 },]);
        assert_eq!(interval_store.insert_range(21..=49), Interval::new_unchecked(21, 49).run_len());
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 21, end: 70 },]));
    }
    ...
    fn insert_range_concescutive_begin_end() {
        ...
        assert_eq!(interval_store.insert_range(21..=49), Interval::new_unchecked(21, 49).run_len());
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 10, end: 70 },]));
    }
    ...
    fn insert_range_begin_overlap_concescutive_end() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval::new_unchecked(2, 700)]));
    }

    #[test]
    fn insert_range_pin_1() {
        let mut interval_store = IntervalStore(std::vec![Interval::new_unchecked(65079, 65079)]);
        assert_eq!(interval_store.insert_range(65080..=65080), 1);
        assert_eq!(
            interval_store,
            IntervalStore(std::vec![Interval::new_unchecked(65079, 65080)])
        );
    }
    ...
    fn push_empty() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 80, end: 80 },]));
    }
    ...
    fn push_new_max_consecutive() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 50, end: 71 },]));
    }
    ...
    fn push_existing_u16_max() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 50, end: u16::MAX },]);
    ...
    fn push_new_u16_max() {
        let mut interval_store =
            IntervalStore(std::vec![Interval { start: 50, end: u16::MAX - 1 },]);
        assert!(interval_store.push(u16::MAX));
        assert_eq!(
            interval_store,
            IntervalStore(std::vec![Interval { start: 50, end: u16::MAX },])
        );
    }

    #[test]
    fn remove_end_of_interval() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 50, end: 60 },]);
        assert!(interval_store.remove(60));
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 50, end: 59 },]));
    }
    ...
    fn remove_begin_of_interval() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 51, end: 60 },]));
    }

    #[test]
    fn remove_middle() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 1, end: 3 },]);
    ...
    fn remove_nothing() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 50, end: 60 },]));
    }
    ...
    #[test]
    fn remove_interval() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 50, end: 50 },]);
        assert!(interval_store.remove(50));
        assert_eq!(interval_store, IntervalStore(std::vec![]));
    }

    #[test]
    fn remove_range_exact_one() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 40, end: 60 },]);
    ...
    fn remove_range_begin_exact_overlap_end_one() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 81, end: 90 },]));
    }
    ...
    fn remove_range_begin_overlap_end_exact_one() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 40, end: 49 },]));
    }
    ...
    fn remove_range_begin_no_overlap_end_exact_one_2() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval::new_unchecked(700, 900),]));
    }
    ...
    fn remove_range_end_overlap_many() {
        ...
        assert_eq!(interval_store, IntervalStore(std::vec![Interval { start: 851, end: 900 },]));
    }
    ...
    fn remove_range_complete_overlap() {
        let mut interval_store = IntervalStore(std::vec![Interval { start: 51, end: 6000 },]);
        assert_eq!(
    ...
    fn remove_smallest_many_2() {
        ...
        assert_eq!(
            interval_store,
            IntervalStore(std::vec![Interval::new_unchecked(4200, 6000),])
        );
    }
    ...
    fn remove_biggest_many_2() {
        ...
        interval_store.remove_biggest(500);
        assert_eq!(interval_store, IntervalStore(std::vec![Interval::new_unchecked(1, 5800),]));
    }

    #[test]
    fn contains_index_1() {
        let interval_store = IntervalStore(std::vec![
    ...
    #[test]
    fn is_disjoint_1() {
        let interval_store_1 = IntervalStore(std::vec![
            Interval { start: 1, end: 600 },
            Interval { start: 1401, end: 1600 },
            Interval { start: 15901, end: 16000 },
        ]);
        let interval_store_2 = IntervalStore(std::vec![Interval { start: 601, end: 1200 },]);
    ...
    fn is_disjoint_2() {
        ...
        let interval_store_2 = IntervalStore(std::vec![Interval { start: 600, end: 1200 },]);
        assert!(!interval_store_1.is_disjoint(&interval_store_1));
        assert!(!interval_store_2.is_disjoint(&interval_store_2));
        assert!(!interval_store_1.is_disjoint(&interval_store_2));
        assert!(!interval_store_2.is_disjoint(&interval_store_1));
    ...
    fn is_disjoint_3() {
        ...
        let interval_store_2 = IntervalStore(std::vec![Interval { start: 15800, end: 15905 },]);
        assert!(!interval_store_1.is_disjoint(&interval_store_1));
        assert!(!interval_store_2.is_disjoint(&interval_store_2));
        assert!(!interval_store_1.is_disjoint(&interval_store_2));
        assert!(!interval_store_2.is_disjoint(&interval_store_1));
    ...
    fn is_disjoint_array_store_1() {
        let array_store = ArrayStore::from_vec_unchecked(std::vec![0, 60, 200, 500,]);
        let interval_store = IntervalStore(std::vec![Interval { start: 70, end: 199 },]);
    ...
    fn is_disjoint_array_store_2() {
        let array_store = ArrayStore::from_vec_unchecked(std::vec![0, 60, 200, 500,]);
        let interval_store = IntervalStore(std::vec![Interval { start: 1, end: 400 },]);
        assert!(!interval_store.is_disjoint_array(&array_store));
    }
    ...
    #[test]
    fn is_subset_1() {
        let interval_store_1 = IntervalStore(std::vec![Interval { start: 1500, end: 1600 },]);
        let interval_store_2 = IntervalStore(std::vec![
            Interval { start: 1, end: 600 },
            Interval { start: 1401, end: 1600 },
            Interval { start: 15901, end: 16000 },
        ]);
    ...
    #[test]
    fn is_subset_2() {
        let interval_store_1 = IntervalStore(std::vec![Interval { start: 50, end: 700 },]);
        let interval_store_2 = IntervalStore(std::vec![
            Interval { start: 1, end: 600 },
            Interval { start: 1401, end: 1600 },
            Interval { start: 15901, end: 16000 },
    ...
    fn intersection_len_3() {
        let interval_store_1 = IntervalStore(std::vec![Interval { start: 1, end: 2000 },]);
        let interval_store_2 = IntervalStore(std::vec![Interval { start: 1001, end: 3000 },]);
    ...
    fn intersection_len_bitmap_1() {
        ...
        let interval_store_1 = IntervalStore(std::vec![Interval { start: 20, end: 600 },]);
        let intersect_len = 4;
    ...
    fn intersection_len_bitmap_6() {
        ...
        let interval_store_1 = IntervalStore(std::vec![Interval { start: 64, end: 64 },]);
    ...
    fn intersection_len_array_1() {
        let array_store = ArrayStore::from_vec_unchecked(std::vec![20, 40, 60, 500, 5001]);
        let interval_store_1 = IntervalStore(std::vec![Interval { start: 20, end: 600 },]);
        let intersect_len = 4;
    ...
    #[test]
    fn min_0() {
        let interval_store = IntervalStore(std::vec![Interval::new_unchecked(20, u16::MAX)]);
    ...
    #[test]
    fn union_1() {
        let mut interval_store_1 = IntervalStore(std::vec![
            Interval::new_unchecked(0, 0),
            Interval::new_unchecked(2, 11),
            Interval::new_unchecked(5000, 7000),
            Interval::new_unchecked(8000, 10000),
    ...
    #[test]
    fn union_array() {
        let mut values = std::vec![0, 1, 2, 3, 4, 2000, 5000, u16::MAX];
        values.sort();
    ...
    fn symmetric_difference_1() {
        let interval_store_1 = IntervalStore(std::vec![Interval::new_unchecked(0, 50),]);
        let interval_store_2 = IntervalStore(std::vec![Interval::new_unchecked(100, 200),]);
        assert_eq!(
            &interval_store_1 ^ &interval_store_2,
            IntervalStore(std::vec![
    ...
    fn symmetric_difference_2() {
        ...
        let interval_store_2 = IntervalStore(std::vec![Interval::new_unchecked(0, 6000),]);
        assert_eq!(
            &interval_store_1 ^ &interval_store_2,
            IntervalStore(std::vec![
                Interval::new_unchecked(51, 499),
    ...
    #[test]
    fn iter_u16_max() {
        let interval_store = IntervalStore(std::vec![Interval::new_unchecked(0, u16::MAX),]);
